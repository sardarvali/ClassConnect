...
class ChatFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val fabBottomMargin =
            (binding.fabScrollDown.layoutParams as ViewGroup.MarginLayoutParams).bottomMargin
        ViewCompat.setOnApplyWindowInsetsListener(binding.root) { _, insets ->
            ...
            binding.inputLayout.updateLayoutParams<ViewGroup.MarginLayoutParams> {
                bottomMargin = dp12 + bottomInset
            }
            binding.fabScrollDown.updateLayoutParams<ViewGroup.MarginLayoutParams> {
                bottomMargin = fabBottomMargin + bottomInset
            }
            binding.rvMessages.updatePadding(bottom = bottomInset + binding.inputLayout.height + 24)
            insets
        }
        ...
        viewModel.clearResult.observe(viewLifecycleOwner) { result ->
            when (result) {
                is NetworkResult.Success -> Snackbar.make(
                    binding.root,
                    getString(R.string.chat_cleared),
                    Snackbar.LENGTH_SHORT
                ).show()

                is NetworkResult.Error -> Snackbar.make(
                    binding.root,
                    result.message,
                    Snackbar.LENGTH_SHORT
                ).show()
            ...
            override fun onMenuItemSelected(menuItem: MenuItem): Boolean {
                return when (menuItem.itemId) {
                    R.id.action_chat_mute -> {
                        ...
                        val msg =
                            if (!muted) getString(R.string.chat_muted) else getString(R.string.chat_unmuted)
    ...
    private fun showClassCodeDialog() {
        ...
        MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton(getString(R.string.copy_to_clipboard)) { _, _ ->
                ...
                Snackbar.make(
                    binding.root,
                    getString(R.string.class_code_copied),
                    Snackbar.LENGTH_SHORT
                ).show()
            }
    ...
    private fun showClearChatConfirmDialog() {
        ...
        if (user?.role == Constants.ROLE_STUDENT) {
            Snackbar.make(
                binding.root,
                "Only teachers can clear chat history",
                Snackbar.LENGTH_SHORT
            ).show()
            return
        }
