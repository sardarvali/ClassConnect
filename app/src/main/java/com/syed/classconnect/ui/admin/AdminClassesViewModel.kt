...
class AdminClassesViewModel @Inject constructor(
    ...
    private fun applyFilter() {
        val filtered =
            if (showUnassigned) cachedClasses.filter { it.teacherId.isNullOrEmpty() || it.teacherName == "[Unassigned]" }
            else cachedClasses
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                db.collection("classes").document(classId)
                    .update(mapOf("teacherId" to teacher.uid, "teacherName" to teacher.name))
                    .await()
