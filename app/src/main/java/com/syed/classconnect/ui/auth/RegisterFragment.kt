...
=======
        listOf(
            ...
            binding.tvRoleLabel,
            binding.rgRole,
            binding.tilInstitutionCode,
            binding.registrationPathCard,
            binding.btnRegister,
            binding.tvLogin
        ).animateEntrance(startDelay = 80L, stepDelay = 45L)

        binding.btnRegister.addPressEffect()
        binding.tvLogin.addPressEffect()
...
=======
            override fun afterTextChanged(s: Editable?) {
                updatePathCard(s?.toString()?.trim() ?: "")
            }
...
=======
        if (!ValidationUtils.isNotEmpty(name)) {
            binding.tilName.error = getString(R.string.error_empty_field); return
        }
        binding.tilName.error = null
        if (!ValidationUtils.isValidEmail(email)) {
            binding.tilEmail.error = getString(R.string.error_invalid_email); return
        }
>>>>>>> final
...
=======
        if (!ValidationUtils.doPasswordsMatch(password, confirm)) {
            binding.tilConfirmPassword.error =
                getString(R.string.error_passwords_dont_match); return
        }
>>>>>>> final
...
=======
        if (loading) {
            binding.progressBar.show(); binding.btnRegister.isEnabled = false
        } else {
            binding.progressBar.hide(); binding.btnRegister.isEnabled = true
        }
    }
