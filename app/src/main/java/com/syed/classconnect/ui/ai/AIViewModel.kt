...
=======
    private val systemPrompt =
        "You are an AI Study Buddy for a student. Help with explanations, practice problems, concept summaries, and study tips. Be concise, friendly, and educational."
>>>>>>> final
...
>>>>>>> final
    ...
    fun generateQuizQuestions(topic: String, onResult: (String) -> Unit) {
        ...
        viewModelScope.launch {
<<<<<<< HEAD
            val prompt = "Generate 5 multiple choice questions about '$topic'. For each question provide: the question, 4 options (A, B, C, D), and the correct answer. Format as JSON array."
=======
            val prompt = """
    ...
    fun createAssignmentAndPostFromAI(
        ...
        viewModelScope.launch {
            ...
            createResult.fold(
                ...
                onFailure = {
                    _actionResult.value =
                        AIActionResult.Error(it.message ?: "Failed to create assignment")
                }
            )
        }
    }
    ...
    fun createQuizFromAI(
        ...
        viewModelScope.launch {
            ...
            if (aiQuestions.isEmpty()) {
                _actionResult.value =
                    AIActionResult.Error("Could not parse quiz questions. Try again.")
                return@launch
            }
            val quiz = Quiz(
                title = quizTitle,
