...
=======
                if (s.toString() != code) {
                    binding.etClassCode.setText(code); binding.etClassCode.setSelection(code.length)
                }
>>>>>>> final
                if (code.length == 6) {
...
=======
            if (code.length != 6) {
                binding.tilClassCode.error =
                    "Enter a valid 6-character code"; return@setOnClickListener
            }
>>>>>>> final
...
=======
                is NetworkResult.Loading -> {
                    binding.progressBar.show(); binding.btnJoin.isEnabled = false
                }
