...
class MainActivity : AppCompatActivity() {
    ...
    override fun onStart() {
        ...
        if (wasInBackground && !initialLaunch) {
            ...
            if (timeInBackground >= minBackgroundDuration && !isLocked) {
                checkBiometricLock()
            }
        }
    ...
    override fun onStop() {
        super.onStop()
        if (!isChangingConfigurations && !isLocked) {
            wasInBackground = true
            backgroundTimestamp = System.currentTimeMillis()
        } else if (isLocked) {
            wasInBackground = false // Prevent lock loop if prompt causes onStop
        }
    }

    private fun checkBiometricLock() {
        val prefs = getSharedPreferences("classconnect_prefs", MODE_PRIVATE)
