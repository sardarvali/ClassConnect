...
class FeedViewModel @Inject constructor(
        ...
        viewModelScope.launch {
            try {
                feedRepository.getAnnouncements(classId).collect { announcements ->
                    val sorted = announcements.sortedWith(compareByDescending<Announcement> { it.isPinned }
    ...
    fun postAnnouncement(classId: String, title: String, body: String, authorId: String, authorName: String) {
        viewModelScope.launch {
            feedRepository.postAnnouncement(classId, Announcement(
