        ...
        viewModelScope.launch {
            ...
            launch {
                try {
                    ...
                    _todayClasses.value =
                        classes.filter { cls ->
                            ScheduleUtils.isScheduledToday(cls.schedule)
                        }.sortedWith(
                            compareBy<ClassRoom> { ScheduleUtils.todayStartMinutes(it.schedule) ?: Int.MAX_VALUE }
            ...
            try {
                ...
                if (response.isSuccessful) {
                    val articles = response.body()?.articles
                        ...
                        ?: emptyList()
                    if (articles.isEmpty()) {
                        _newsStatus.value = "No live education news found right now. Showing fallback articles."
                        _news.value = getFallbackNews()
                    } else {
                    ...
                    _newsStatus.value = "Unable to fetch live news right now. Showing fallback articles."
                    _news.value = getFallbackNews()
                }
            } catch (e: Exception) {
