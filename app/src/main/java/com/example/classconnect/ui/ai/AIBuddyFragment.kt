...
class AIBuddyFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val fabBottomMargin = (binding.fabUseResponse.layoutParams as ViewGroup.MarginLayoutParams).bottomMargin
        val progressBottomMargin = (binding.progressBar.layoutParams as ViewGroup.MarginLayoutParams).bottomMargin
        ...
        binding.btnSend.setOnClickListener {
            ...
            if (input.isNotEmpty()) { binding.etInput.setText(""); binding.layoutSuggestions.hide(); viewModel.sendMessage(input) }
        }
        ...
        binding.btnClear.setOnClickListener { viewModel.clearConversation(); adapter.submitList(emptyList()); binding.layoutSuggestions.show(); binding.fabUseResponse.hide() }
        binding.fabUseResponse.setOnClickListener {
            val lastAiText = viewModel.messages.value?.lastOrNull { !it.isUser }?.text ?: return@setOnClickListener
        ...
        viewModel.messages.observe(viewLifecycleOwner) { messages ->
            ...
            val isTeacher = viewModel.userRole.value == Constants.ROLE_TEACHER || viewModel.userRole.value == Constants.ROLE_ADMIN
            ...
            if (isTeacher && shouldShowPlanActions && latestAiText != lastAutoPromptedAiText) {
                ...
                AIActionsSheet.newInstance(latestAiText, "AI Lesson Plan", AIActionsSheet.MODE_LESSON_PLAN)
                    .show(childFragmentManager, "ai_actions")
            }
        }
        ...
        viewModel.error.observe(viewLifecycleOwner) { error -> error ?: return@observe; adapter.addErrorMessage(error) }
    }
    private fun setupSuggestions() {
        val suggestions = listOf("Explain this concept","Create practice questions","Summarize my notes","Help me study for exam")
        val chips = listOf(binding.chip1, binding.chip2, binding.chip3, binding.chip4)
        suggestions.forEachIndexed { i, text -> chips.getOrNull(i)?.apply { this.text = text; setOnClickListener { binding.etInput.setText(text) } } }
    }
