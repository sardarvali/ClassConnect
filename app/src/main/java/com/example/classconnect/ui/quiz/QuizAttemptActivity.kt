...
class QuizAttemptActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.quizDetail.observe(this) { quiz ->
            ...
            binding.btnPrev.setOnClickListener {
                if (currentQuestion > 0) { currentQuestion--; showQuestion(quiz, currentQuestion) }
            }
            binding.btnNext.setOnClickListener {
                if (currentQuestion < quiz.questions.size - 1) { currentQuestion++; showQuestion(quiz, currentQuestion) }
            }
    ...
    private fun showQuestion(quiz: com.syed.classconnect.data.model.Quiz, index: Int) {
        ...
        binding.tvProgress.text = getString(com.syed.classconnect.R.string.question_of, index + 1, quiz.questions.size)
        binding.progressBar.max = quiz.questions.size
        binding.progressBar.progress = index + 1

        val optionViews = listOf(binding.rbOption0, binding.rbOption1, binding.rbOption2, binding.rbOption3)
        ...
        binding.btnSubmit.visibility = if (index == quiz.questions.size - 1) View.VISIBLE else View.GONE
    }
    ...
    private fun startTimer(totalSeconds: Long) {
        timer = object : CountDownTimer(totalSeconds * 1000, 1000) {
            override fun onTick(ms: Long) {
                val secs = ms / 1000
                binding.tvTimer.text = getString(com.syed.classconnect.R.string.time_remaining, formatDuration(secs))
    ...
    private fun submitQuiz() {
        ...
        viewModel.submitResult.observe(this) { result ->
            if (result is NetworkResult.Success) {
                val fragment = QuizResultFragment.newInstance(classId, quizId, uid, score, quiz.totalMarks)
                fragment.show(supportFragmentManager, "quiz_result")
                supportFragmentManager.setFragmentResultListener("quiz_result_dismissed", this) { _, _ ->
                    finish()
                }
            }
        }
        ...
        firestore.collection("classes/$classId/quizzes/$quizId/attempts")
            ...
            .addOnSuccessListener { doc ->
                if (doc.exists() && doc.getBoolean("isDraft") == true) {
                    ...
                    val saved = doc.get("answers") as? Map<String, Long> ?: return@addOnSuccessListener
                    ...
                    com.google.android.material.snackbar.Snackbar.make(
                        binding.root, "Resuming saved progress", com.google.android.material.snackbar.Snackbar.LENGTH_SHORT
                    ).show()
                }
            }
    }
